...
export default class TDObjectCoder {  
  ...
  public static get(): TDObjectCoder { return TDObjectCoder.it; }

  public encode(obj: any, opt = new ObjectCodeOption, target = new TreeDoc().root, ctx = new ObjectCoderContext()): TDNode {
  ...
  private setRef(node: TDNode, ref: string) {
    node.setType(TDNodeType.MAP).createChild(this.KEY_REF).setValue(ref);
    return node;
  }
